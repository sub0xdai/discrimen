#!/usr/bin/env bash
# ponytail: bash next to its subject, matching jev.sh and jev_sieve_report.sh.
set -uo pipefail

JEV_SH="${JEV_SH:-$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/jev.sh}"
PORT=$(( 21000 + ($$ % 20000) ))
...
missing=""
for dep in jq curl nc pgrep; do
  command -v "$dep" >/dev/null 2>&1 || missing="$missing $dep"
done
if [[ -n "$missing" ]]; then
  printf 'check-argv: cannot run, missing:%s\n' "$missing" >&2
  printf 'check-argv: needs Linux (reads /proc) and GNU netcat (-l -p)\n' >&2
  exit 2
fi
if [[ ! -x "$JEV_SH" ]]; then
  printf 'check-argv: no transport at %s\n' "$JEV_SH" >&2
  exit 2
fi
